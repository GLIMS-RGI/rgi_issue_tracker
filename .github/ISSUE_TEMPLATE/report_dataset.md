---
name: Related dataset
about: Report a dataset derived from and adding value to the RGI for listing on https://rgidata.org/products
title: "New dataset: "
labels: documentation
assignees: ''
---
**Briefly explain what the dataset is and why it should be included in the list.**

*Note that we will only list open-access datasets covering at least one RGI region
that are directly derived from and clearly extend or enrich the RGI —
for example, a new ice velocity or debris cover product.*
